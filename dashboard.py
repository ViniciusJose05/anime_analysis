...
def cria_pagina_dashboard(df_inicial, df_explodidos, generos_usados):
    ...
    with colg1:
        ...
        fig3 = px.bar(studio_avg_simples.head(15).to_pandas().sort_values("Nota Média"), x='Nota Média', y='Studios',
                      orientation='h', color_discrete_sequence=px.colors.sequential.Viridis_r)
        grafico_card("Estúdios com Melhores Notas", fig3)
    with colg2:
        fig2 = px.line(genero_score.head(15).to_pandas(), x='Genres', y='Nota Média', markers=True,
                       color_discrete_sequence=px.colors.sequential.Viridis_r)
    ...
    fig5 = px.scatter(relacao_pop, x='Score', y='Members', size='Members', color='Score', hover_name='Name',
